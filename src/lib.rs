...
pub enum Message {
    ...
    FlagClaimStatus{ flagsClaimed: Vec<ClaimStatus> }, 
    FlagStatus{flagNum:u32, direction:Direction, cards:Vec<Card>},
    OpponentPlay{ number: i32, card: Card },
    PlayerHand{ direction:Direction, cards: Vec<Card>},
    PlayCard,
}
...
pub fn parse_message(message: String) -> Message {
    ...
    match &split[..] {
        ...
        &["player", direction, "hand", ref cards..] if cards.len() <= 7 && (direction == "north" || direction == "south") => {
            let mut playerCards = Vec::new();
            for i in cards {
                playerCards.push(convert_string_to_card(i));
            }
            Message::PlayerHand{ direction:convert_direction(direction), cards:playerCards}
        },
        &["flag", flag, "cards", direction, ref cards..] if flag >= "1" && flag <= "9" && (direction == "north" || direction == "south")  => {
            let mut flagsCards = Vec::new();
            let a = flag.parse::<u32>().unwrap();
            for i in cards {
                flagsCards.push(convert_string_to_card(i));
            }
            Message::FlagStatus{flagNum:a, direction:convert_direction(direction), cards:flagsCards}
        },
        ...
        &["opponent", "play", flag, card] if flag >= "1" && flag <= "9" => {
            ...
            Message::OpponentPlay{ number: a, card: convert_string_to_card(card)}
        },
        ...
        &["player", direction, hand,  ref cards..] => {
            Message::Blank
        },
        &["flag", number, "cards", direction, ref cards..] => {
            Message::Blank
        },
        _ => Message::Blank,
    }
...
mod test_parsing_messages{
    ...
    fn flag_claim_message() {
        let claimVec = vec![ClaimStatus::North, ClaimStatus::South, ClaimStatus::Unclaimed,
                            ClaimStatus::Unclaimed, ClaimStatus::South, ClaimStatus::North,
                            ClaimStatus::South, ClaimStatus::North, ClaimStatus::Unclaimed];
    ...
    fn flag_card_message() {
        let cards = vec![Card{color:String::from("red"), number:3}, Card{color:String::from("blue"), number:9}];
    ...
    fn player_hand_message() {
        let cards = vec![Card{color:String::from("red"), number:3}, Card{color:String::from("blue"), number:6},
                         Card{color:String::from("green"), number:4}];
